  ...
  return (
    <div className="space-y-2" data-testid="ckd-qualifier">
      ...
      <p className="text-xs text-muted-foreground">
        Qualifier for the algorithm v2.0 special scenario (advanced CKD or suspected
        CKD-MBD). Stage and mineral-bone status are documented here; the engine still
        uses one yes/no/unknown flag. Not an automatic risk upgrade or anabolic route.
        FRAX stays separate.
      </p>
      <div
        role="radiogroup"
      ...
      {advanced ? (
        ...
        >
          {ckdQualifierLabel(value)} — special-scenario review is active for advanced CKD /
          CKD-MBD. Individualize calcium, phosphate, PTH and ALP; not an automatic upgrade.
        </p>
      ) : (
      ...
      {noneButLowCrcl ? (
        <p className="rounded-md border border-border/60 bg-muted/30 px-2 py-1.5 text-xs text-foreground">
          CrCl is &lt; 30 mL/min but the qualifier is “not advanced”. The algorithm follows
          the qualifier (no special scenario) until you pick G4 / G5 / dialysis or CKD-MBD.
        </p>
      ) : null}
    </div>
  );
