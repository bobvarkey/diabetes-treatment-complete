...
interface Props {
  ...
  onChange: (
    key: "fraxAboveNationalThreshold" | "clinicalReviewComplete",
    value: TriState | boolean,
  ) => void;
  onOpenFrax?: () => void;
  defaultOpen?: boolean;
}
...
export default function OsteoporosisAlgorithmPanel({
  input,
  onChange,
  onOpenFrax,
  defaultOpen = true,
  ...
  return (
    ...
    >
      <Callout tone="info" title="Clinician-reviewed decision support">
        {ALGORITHM_PURPOSE} Unknown answers are never treated as negative. The FRAX calculator stays
        in its own sidebar entry; record only whether a country-appropriate result is above the
        applicable national treatment threshold.
      </Callout>
      ...
      <p className="text-xs text-muted-foreground">
        Compute probabilities in the separate FRAX tool, apply the country-specific intervention
        threshold, then record the comparison here. Do not enter raw FRAX percentages on this
        screen.
      </p>
      ...
      <label className="flex items-start gap-2 rounded-md border border-border/60 bg-card/40 px-2 py-1.5 text-sm">
        ...
        />
        <span>
          Special-scenario clinical review is complete (required before assigning below-threshold).
        </span>
      </label>
          ...
          {decision.specialScenariosPresent.map((s) => (
            <div key={s.id} className="rounded-md border border-border/60 bg-card/40 p-3 space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {s.automaticUpgrade ? (
                  <Pill tone="danger">Automatic upgrade</Pill>
                ) : (
                  <Pill tone="default">No automatic upgrade</Pill>
                )}
              </div>
      ...
      {decision.assessmentIncompleteReasons.length > 0 &&
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Very-high-risk pathways consider bone-forming (RAT) agents first. High-risk pathways
          prefer antiresorptives (BD) if suitable. This figure is teaching support, not a
          prescribing order.
        </p>
        <RatBdTeachingFigure />
      </div>
      ...
      <div className="rounded-md border border-border/60 p-3 space-y-2">
        ...
        {decision.drugSelection.preferred ? (
          <KeyRow k="Preferred" v={decision.drugSelection.preferred} />
        ) : null}
        {decision.drugSelection.alternative ? (
          <KeyRow k="Alternative" v={decision.drugSelection.alternative} />
        ) : null}
        {decision.drugSelection.sequence ? (
          <KeyRow k="Sequence" v={decision.drugSelection.sequence} />
        ) : null}
      ...
      <div className="flex items-start gap-2 text-xs text-muted-foreground">
        ...
        <p>
          Educational reference only. Confirm local approvals, contraindications and the official
          country-calibrated FRAX tool before any prescribing decision.
        </p>
      </div>
    </SectionCard>
  );
